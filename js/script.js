...
const formatID = (dateStr) => new Date(dateStr).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });

// 🔍 Helper: Estimasi Baca
...
if (document.getElementById('articleGrid')) {
  async function loadArticles() {
    ...
    if (error) {
      console.error(error);
      document.getElementById('featured').innerHTML = `<p style="color:var(--text-secondary);font-family:var(--font-ui);padding:2rem 0">Gagal memuat artikel.</p>`;
...
if (document.getElementById('articleBody')) {
  ...
  async function loadArticle() {
    if (!slug || slug === 'index.html' || slug.includes('.')) return;
    ...
    if (error || !data) {
      document.getElementById('articleContainer').innerHTML = `
        <div style="text-align:center; padding:5rem 1rem;">
          ...
          <p style="color:var(--text-secondary); margin-bottom:1.5rem">URL yang Anda tuju mungkin sudah tidak tersedia.</p>
          <a href="/" style="font-family:var(--font-ui); font-weight:700; color:var(--accent); text-decoration:underline;">← Kembali ke Beranda</a>
        </div>`;
      return;
    }
    ...
    document.getElementById('coverImg').src = data.cover_image_url || '/placeholder.jpg';
    ...
    ).join('') || '<li style="color:var(--text-secondary)">Tidak ada artikel terkait.</li>';
  }
  loadArticle();
}
